...
export function mapLearningSummary(dto: UserLearningSummaryResponseDto): UserLearningSummary {
  return { ...dto };
}
/** Scalar quiz statistics cevabını frontend hesabı eklemeden taşır. */
export function mapQuizStatistics(dto: QuizStatisticsResponseDto): QuizStatistics {
  return { ...dto };
}
/** Nullable difficult item listesini boş diziye normalize eder. */
export function mapDifficultItems(
  dto: DifficultLearningItemsPagedResponseDto,
): DifficultLearningItemsPage {
  return { ...dto, items: dto.items ?? [] };
}
/** Nullable deck statistics listesini boş diziye normalize eder. */
export function mapDeckStatistics(dto: DeckStatisticsResponseDto): DeckStatistics {
  return { ...dto, items: dto.items ?? [] };
}
/** Nullable confidence bucket listesini boş diziye normalize eder. */
export function mapConfidenceDistribution(
  dto: ConfidenceScoreDistributionResponseDto,
): ConfidenceScoreDistribution {
  return { ...dto, buckets: dto.buckets ?? [] };
}

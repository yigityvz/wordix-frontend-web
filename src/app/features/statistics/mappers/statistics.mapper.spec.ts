...
describe('statistics mappers', () => {
  ...
  it('normalizes difficult items', () => {
    const dto = {
      ...
      hasNextPage: false,
    } as DifficultLearningItemsPagedResponseDto;
    expect(mapDifficultItems(dto).items).toEqual([]);
  });
  /** Deck null listesini state için boş diziye dönüştürür. */
  it('normalizes deck statistics', () => {
    const dto = {
      totalDeckCount: 0,
      activeDeckCount: 0,
      generatedAt: '2026-01-01',
      items: null,
    } as DeckStatisticsResponseDto;
    expect(mapDeckStatistics(dto).items).toEqual([]);
  });
  /** Confidence null bucket listesini state için boş diziye dönüştürür. */
  it('normalizes confidence buckets', () => {
    const dto = {
      totalItemCount: 0,
      averageConfidenceScore: 0,
      generatedAt: '2026-01-01',
      buckets: null,
    } as ConfidenceScoreDistributionResponseDto;
    expect(mapConfidenceDistribution(dto).buckets).toEqual([]);
  });
});

...
@Component({
  selector: 'wx-multiple-choice-question',
  imports: [Button],
...
export class MultipleChoiceQuestion {
  ...
  protected selectOption(id: string): void {
    if (!this.disabled()) this.selectedOptionId.set(id);
  }
  /** Geçerli seçimi parent sayfanın gerçek mutationına iletir. */
  protected submitAnswer(): void {
    const id = this.selectedOptionId();
    if (!this.disabled() && id) this.answerRequested.emit(id);
  }
}

...
@Component({ selector: 'wx-question-progress', templateUrl: './question-progress.html', changeDetection: ChangeDetectionStrategy.OnPush })
export class QuestionProgress {
  ...
  protected readonly percentage = computed(() => this.total() <= 0 ? 0 : Math.min(100, Math.max(0, (this.answered() / this.total()) * 100)));
}

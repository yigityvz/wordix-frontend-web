...
describe('QuizPlayPage', () => {
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            session,
            latestAnswer,
            isSubmittingAnswer: signal(false),
            answerError: signal(null),
            answeredQuestionCount,
            submitAnswer,
            clearAnswerState,
            isSavingRecommendation: signal(false),
            savedRecommendation: signal(null),
            recommendationSaveError: signal(null),
            saveRecommendation,
            clearRecommendationSaveState,
          },
        },
      ],
    });
  ...
  it('submits a canonical multiple-choice answer', () => {
    const option = (fixture.nativeElement as HTMLElement).querySelectorAll(
      'button',
  ...
  it('saves an eligible recommendation with its backend id', () => {
    latestAnswer.set({
      ...
      canAddRecommendedItemToDictionary: true,
      quizRecommendationItemId: 'recommendation-1',
      recommendationReason: 'Needs practice',
    });
    fixture.detectChanges();
    const saveButton = Array.from(
      (fixture.nativeElement as HTMLElement).querySelectorAll('button'),
    ).find((button) => button.textContent?.includes('Save to dictionary'))!;
    saveButton.click();
    ...
    questions: [
      {
        ...
        options: [{ quizOptionId: 'option-1', displayOrder: 1, optionText: 'Merhaba' }],
      },
      {
        quizQuestionId: 'question-2',
        questionOrder: 2,
        questionText: 'Write hello',
        learningItemId: 'item-2',
        wordId: 'word-2',
        phraseId: null,
        sentenceId: null,
        itemType: 'Word',
        questionType: 'TranslateToTargetLanguage',
        isSystemRecommended: false,
        recommendationReason: null,
        quizRecommendationItemId: null,
        options: [],
...
function createAnswer(): QuizAnswerResult {
  return {
    ...
    userAnswerText: null,
    selectedOptionText: 'Merhaba',
    correctAnswerText: 'Merhaba',
    questionResponseTimeInMilliseconds: 1000,
    answeredAt: '2026-01-01T00:00:01Z',
    correctCount: 1,
    wrongCount: 0,
    consecutiveCorrectCount: 1,
    consecutiveWrongCount: 0,
    previousLearningStatus: 'New',

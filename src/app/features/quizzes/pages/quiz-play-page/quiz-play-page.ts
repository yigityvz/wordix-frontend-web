...
export class QuizPlayPage {
  ...
  protected readonly session = computed(() => {
    const value = this.quizFacade.session();
    return value?.quizSessionId === this.routeSessionId ? value : null;
  });
  ...
  protected readonly feedback = computed(() => {
    ...
    const result = this.quizFacade.latestAnswer();
    return question && result?.quizQuestionId === question.quizQuestionId ? result : null;
  });
  /** Mevcut feedback recommendationına ait başarılı save sonucunu seçer. */
  protected readonly savedRecommendation = computed(() => {
    const feedback = this.feedback();
    const saved = this.quizFacade.savedRecommendation();
    return feedback?.quizRecommendationItemId &&
      saved?.quizRecommendationItemId === feedback.quizRecommendationItemId
      ? saved
      : null;
  });
  /** Option varlığını multiple-choice contractı sayar. */
  protected readonly isMultipleChoice = computed(
    () => (this.currentQuestion()?.options.length ?? 0) > 0,
  );
  /** Mevcut sorunun son soru olup olmadığını belirler. */
  protected readonly isLastQuestion = computed(
    () => !!this.session() && this.currentIndex() === this.session()!.questions.length - 1,
  );
  ...
  protected submitMultipleChoice(selectedQuizOptionId: string): void {
    ...
    this.quizFacade.submitAnswer(this.routeSessionId, {
      quizQuestionId: question.quizQuestionId,
      selectedQuizOptionId,
      userAnswer: null,
      questionResponseTimeInMilliseconds: this.getResponseTime(),
    });
  }
  ...
  protected saveRecommendation(): void {
    const feedback = this.feedback();
    if (
      !feedback?.canAddRecommendedItemToDictionary ||
      !feedback.quizRecommendationItemId ||
      this.quizFacade.isSavingRecommendation() ||
      this.savedRecommendation()
    )
      return;
  ...
  protected continueToNextQuestion(): void {
    ...
    if (this.isLastQuestion()) {
      void this.router.navigate(['/quizzes', this.routeSessionId, 'summary']);
      return;
    }
  ...
  private getResponseTime(): number {
    return Math.min(600000, Math.max(1, Date.now() - this.questionStartedAt));
  }
}

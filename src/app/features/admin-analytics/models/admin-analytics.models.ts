...
import {
  ...
  ProviderStatItemResponseDto,
  ProviderStatsAnalyticsResponseDto,
  TopSavedLearningItemResponseDto,
  TopSavedLearningItemsAnalyticsResponseDto,
  TopSearchedItemResponseDto,
  TopSearchesAnalyticsResponseDto,
} from './admin-analytics-api.models';
...
export interface TopSearchesAnalytics extends Omit<TopSearchesAnalyticsResponseDto, 'items'> {
  readonly items: readonly TopSearchedItem[];
}
export type TopSavedLearningItem = TopSavedLearningItemResponseDto;
export interface TopSavedAnalytics extends Omit<
  TopSavedLearningItemsAnalyticsResponseDto,
  'items'
> {
  readonly items: readonly TopSavedLearningItem[];
}
export type MostWrongLearningItem = MostWrongLearningItemResponseDto;
export interface MostWrongAnalytics extends Omit<
  MostWrongLearningItemsAnalyticsResponseDto,
  'items'
> {
  readonly items: readonly MostWrongLearningItem[];
}
export type ProviderStatItem = ProviderStatItemResponseDto;
export interface ProviderStatsAnalytics extends Omit<ProviderStatsAnalyticsResponseDto, 'items'> {
  readonly items: readonly ProviderStatItem[];
}

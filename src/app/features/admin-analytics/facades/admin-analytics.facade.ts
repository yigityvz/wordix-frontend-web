...
export class AdminAnalyticsFacade {
  ...
  readonly topSearchesStatus = this.store.selectSignal(AdminAnalyticsSelectors.selectTopSearchesStatus);
  readonly topSearches = this.store.selectSignal(AdminAnalyticsSelectors.selectTopSearches);
  readonly topSearchesQuery = this.store.selectSignal(AdminAnalyticsSelectors.selectTopSearchesQuery);
  readonly topSearchesError = this.store.selectSignal(AdminAnalyticsSelectors.selectTopSearchesError);
  ...
  readonly providerStatsStatus = this.store.selectSignal(AdminAnalyticsSelectors.selectProviderStatsStatus);
  readonly providerStats = this.store.selectSignal(AdminAnalyticsSelectors.selectProviderStats);
  readonly providerStatsQuery = this.store.selectSignal(AdminAnalyticsSelectors.selectProviderStatsQuery);
  readonly providerStatsError = this.store.selectSignal(AdminAnalyticsSelectors.selectProviderStatsError);
  readonly isAnyLoading = this.store.selectSignal(AdminAnalyticsSelectors.selectIsAnyAdminAnalyticsLoading);

  /** Admin dashboard endpoint intentini opsiyonel tarih aralığıyla gönderir. */
  loadDashboard(query: AdminAnalyticsDateRangeQuery = {}): void { this.store.dispatch(AdminAnalyticsActions.loadDashboard({ query })); }
  /** Top searches endpoint intentini canonical 20 limit ile gönderir. */
  loadTopSearches(query: AdminAnalyticsListQuery = { limit: 20 }): void { this.store.dispatch(AdminAnalyticsActions.loadTopSearches({ query })); }
  /** Top saved endpoint intentini canonical 20 limit ile gönderir. */
  loadTopSaved(query: AdminAnalyticsListQuery = { limit: 20 }): void { this.store.dispatch(AdminAnalyticsActions.loadTopSaved({ query })); }
  /** Most wrong endpoint intentini canonical 20 limit ile gönderir. */
  loadMostWrong(query: AdminAnalyticsListQuery = { limit: 20 }): void { this.store.dispatch(AdminAnalyticsActions.loadMostWrong({ query })); }
  /** Provider stats endpoint intentini opsiyonel tarih aralığıyla gönderir. */
  loadProviderStats(query: AdminAnalyticsDateRangeQuery = {}): void { this.store.dispatch(AdminAnalyticsActions.loadProviderStats({ query })); }
  /** Feature teardown için bütün admin analytics state'ini temizler. */
  clear(): void { this.store.dispatch(AdminAnalyticsActions.clear()); }
}

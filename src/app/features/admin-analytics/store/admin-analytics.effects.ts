...
import {
  ...
  mapTopSaved,
  mapTopSearches,
} from '../mappers/admin-analytics.mapper';
...
export class AdminAnalyticsEffects {
  ...
  readonly loadDashboard$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ query }) =>
        this.api.getDashboard(query).pipe(
          map((dto) =>
            AdminAnalyticsActions.loadDashboardSuccess({ dashboard: mapAdminDashboard(dto) }),
          ),
          catchError((error: unknown) =>
            of(
              AdminAnalyticsActions.loadDashboardFailure({
                message: getAdminAnalyticsError(error, 'Admin dashboard verileri yüklenemedi.'),
              }),
            ),
          ),
        ),
  ...
  readonly loadTopSearches$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AdminAnalyticsActions.loadTopSearches),
      switchMap(({ query }) =>
        this.api.getTopSearches(query).pipe(
          map((dto) =>
            AdminAnalyticsActions.loadTopSearchesSuccess({ analytics: mapTopSearches(dto) }),
          ),
          catchError((error: unknown) =>
            of(
              AdminAnalyticsActions.loadTopSearchesFailure({
                message: getAdminAnalyticsError(error, 'En çok arananlar yüklenemedi.'),
              }),
            ),
          ),
        ),
  ...
  readonly loadTopSaved$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AdminAnalyticsActions.loadTopSaved),
      switchMap(({ query }) =>
        this.api.getTopSaved(query).pipe(
          map((dto) => AdminAnalyticsActions.loadTopSavedSuccess({ analytics: mapTopSaved(dto) })),
          catchError((error: unknown) =>
            of(
              AdminAnalyticsActions.loadTopSavedFailure({
                message: getAdminAnalyticsError(error, 'En çok kaydedilenler yüklenemedi.'),
              }),
            ),
          ),
        ),
  ...
  readonly loadMostWrong$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AdminAnalyticsActions.loadMostWrong),
      switchMap(({ query }) =>
        this.api.getMostWrong(query).pipe(
          map((dto) =>
            AdminAnalyticsActions.loadMostWrongSuccess({ analytics: mapMostWrong(dto) }),
          ),
          catchError((error: unknown) =>
            of(
              AdminAnalyticsActions.loadMostWrongFailure({
                message: getAdminAnalyticsError(error, 'En çok yanlış cevaplananlar yüklenemedi.'),
              }),
            ),
          ),
        ),
  ...
  readonly loadProviderStats$ = createEffect(() =>
    this.actions$.pipe(
      ofType(AdminAnalyticsActions.loadProviderStats),
      switchMap(({ query }) =>
        this.api.getProviderStats(query).pipe(
          map((dto) =>
            AdminAnalyticsActions.loadProviderStatsSuccess({ analytics: mapProviderStats(dto) }),
          ),
          catchError((error: unknown) =>
            of(
              AdminAnalyticsActions.loadProviderStatsFailure({
                message: getAdminAnalyticsError(error, 'Provider istatistikleri yüklenemedi.'),
              }),
            ),
          ),
        ),
